...
# Evolving take 1
# Random: random points, alpha, Re_chord
# Eval:   solver L/D
...
def evolve(population_count, generations, elite_count,
           breed_count, random_function, eval_function,
           breed_function):
...
def randomFunction():
    x_points = [random.random() for _ in range(140)]
    y_points = [random.random() for _ in range(140)]
    foil = Airfoil(0, 0, 0)
    foil.x_points = np.array(x_points)
    foil.y_points = np.array(y_points)
    foil.create_points()
...
def main() -> None:
    ...
    print(best[0])
    print("plotted best")
    best[0].savePlot("take1")
    print("saved diagram")
    best[0].savePoints("take1")
    print(best[1], best[2])
    with open("take1/summary.txt", 'w') as f:
