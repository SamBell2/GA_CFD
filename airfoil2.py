...
class Panel:
    ...
    def __repr__(self):
        ...
        return (f"Panel: ({self.xa:.3f},{self.ya:.3f}) → ({self.xb:.3f},{self.yb:.3f}), "
                f"center=({self.xc:.3f},{self.yc:.3f}), length={self.length:.3f}")


class Airfoil:
    def __init__(self, max_camber_dev=0, max_camber_loc=0, thickness=0, num_points=200):
        self.max_camber_dev = max_camber_dev
        self.max_camber_loc = max_camber_loc
        self.thickness = thickness 
        self.generateAirfoil(max_camber_dev, max_camber_loc, thickness, num_points)
        self.name = "NACA ----"
    ...
    def generateAirfoil(self, max_camber_dev, max_camber_loc, thickness, num_points=100) -> None:
        ...
        if max_camber_dev > 0 and max_camber_loc > 0:
            f = x < max_camber_loc; r = ~f
            yc[f] = max_camber_dev/max_camber_loc**2*(2*max_camber_loc*x[f]-x[f]**2)
            dyc[f] = 2*max_camber_dev/max_camber_loc**2*(max_camber_loc-x[f])
            yc[r] = max_camber_dev/(1-max_camber_loc)**2*((1-2*max_camber_loc)+2*max_camber_loc*x[r]-x[r]**2)
            dyc[r] = 2*max_camber_dev/(1-max_camber_loc)**2*(max_camber_loc-x[r])
        th = np.arctan(dyc)
    ...
    def plot(self) -> None:
        ...
        plt.tight_layout()
        plt.plot(self.x_points, self.y_points, "b-", linewidth=2, label="Surface")
    ...
    def savePoints(self) -> None:
        to_save = '\n'.join([f"{x},{y}" for x, y in zip(self.x_points, self.y_points)])
